<template>
  <v-app id="inspire">
    ...
    >
      <v-card>
        ...
        >
          ...
          ></iframe> -->
          <iframe
            src="http://dt.rozetatech.com:3000/grafana/d/RrGLp3R7z/test_airforce?orgId=1&refresh=5s&kiosk"
            width="100%"
            height="100%"
            frameborder="0"
          ></iframe>
    ...
    >
      <v-card>
        <v-toolbar dark color="black">
          <v-btn icon dark @click="dialog_device = false">
            <v-icon>mdi-close</v-icon>
          </v-btn>
          <v-toolbar-title>장치제어</v-toolbar-title>
          <v-spacer></v-spacer>
        </v-toolbar>
        <v-container
          class="ma-0 pa-0 black"
...
import DeviceCtrl from "@/views/DeviceControl.vue";
  ...
  methods: {
    routeContent(key) {
      if (key === "main") {
        ...
        //this.routePath = DeviceCtrl;
        this.dialog_device = true;
      } else if (key === "UserMng") {
        this.routePath = UserMng;
      } else if (key === "RoleMng") {
        this.routePath = RoleMng;
      }
    },
    /* createConnection() {
